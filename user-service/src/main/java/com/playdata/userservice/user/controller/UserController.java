...
public class UserController {
    ...
    // 컨트롤러는 서비스에 의존하고 있다. (요청과 함께 전달받은 데이터를 서비스에 넘겨야 함!)
    // 빈 등록된 서비스 객체를 자동으로 주입 받자!
    private final UserService userService;
    private final JwtTokenProvider jwtTokenProvider;
    private final RedisTemplate<String, Object> redisTemplate;

    // 기존에는 yml 값 가지고 올 때 @Value를 사용해서 끌고 옴
    ...
    public ResponseEntity<?> userCreate(@Valid @RequestBody UserSaveReqDTO dto) {
        ...
        CommonResDTO resDTO
                = new CommonResDTO(HttpStatus.CREATED, "User Created", saved.getName());
...
//        userService.saveRefreshToken(user.getEmail(), refreshToken);
    ...
    public ResponseEntity<?> getUserByEmail(@RequestParam String email) {
        ...
        CommonResDTO resDTO = new CommonResDTO(HttpStatus.OK, "이메일로 회원 조회", dto);
